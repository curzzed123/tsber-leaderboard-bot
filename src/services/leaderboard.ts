 ...
 * Tries stored ID first, then searches recent messages.
 */
async function findBotMessage(
  ...
  if (storedId) {
    try {
      ...
      if (msg && msg.author.id === client.user!.id) return storedId;
    } catch { /* stale ID */ }
  }
...
async function storeMessageId(channelId: string, messageId: string): Promise<void> {
  ...
  if (lb) {
    ...
    const cfg = LEADERBOARDS.find((l) => l.channelId === channelId);
    if (cfg) {
      guildConfig.leaderboards.push({
        channelId, messageId,
        minRank: cfg.minRank, maxRank: cfg.maxRank, title: '',
      });
    }
  }
...
/**
 * Initialize on startup. Edits existing message or creates one if none exists.
 * Called ONCE on boot.
 */
export async function initLeaderboardMessages(client: Client): Promise<void> {
  for (const lb of LEADERBOARDS) {
    try {
      ...
      const msgId = await findBotMessage(client, lb.channelId, storedId);

      if (msgId) {
        const message = await channel.messages.fetch(msgId);
        await message.edit({ embeds });
        if (msgId !== storedId) await storeMessageId(lb.channelId, msgId);
        logger.info(`Leaderboard ${lb.minRank}-${lb.maxRank}: edited message ${msgId}`);
      } else {
        // First boot — no message exists yet
        const message = await channel.send({ embeds });
        await storeMessageId(lb.channelId, message.id);
        logger.info(`Leaderboard ${lb.minRank}-${lb.maxRank}: created message ${message.id}`);
      }
    } catch (error) {
...
/**
 * Refresh leaderboards — ONLY EDITS, NEVER sends new messages.
 * Called by /setrank, /forcewin, /approve-loa, scheduler, etc.
 */
  ...
  for (const lb of LEADERBOARDS) {
    try {
      ...
      if (msgId) {
        ...
        await message.edit({ embeds });
        if (msgId !== storedId) await storeMessageId(lb.channelId, msgId);
        logger.info(`REFRESH: Leaderboard ${lb.minRank}-${lb.maxRank} edited (message ${msgId})`);
      } else {
        // No message found — this shouldn't happen after init
        // Create one as fallback so the bot doesn't break
        const message = await channel.send({ embeds });
        await storeMessageId(lb.channelId, message.id);
        logger.warn(`REFRESH: No existing message found for ${lb.minRank}-${lb.maxRank}, created new ${message.id}`);
      }
    } catch (error) {
