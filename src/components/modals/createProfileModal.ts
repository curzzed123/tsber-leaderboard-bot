...
export async function handleCreateProfileModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  if (existing) {
    await interaction.reply({ content: `You already created a profile as **${existing.robloxUsername}**. Contact staff if you need to update it.`, ephemeral: true });
    return;
  }
  ...
  if (!robloxData) {
    await interaction.editReply({ content: `No Roblox account found with the username **${robloxUsername}**. Check the spelling and try again.` });
    return;
  }
  ...
  const embed = new (await import('discord.js')).EmbedBuilder()
    .setColor(0x57F287)
    .setDescription(
      `Profile created for **${player.robloxUsername}**\n` +
      `**Region:** ${player.region}\n` +
      `**Status:** Unranked — use \`/setrank\` to assign a spot (1-30)`
    );

  if (finalHeadshotUrl) embed.setThumbnail(finalHeadshotUrl);

  await interaction.editReply({ embeds: [embed] });
  logger.info(`Profile created for ${interaction.user.id} — Roblox: ${robloxData.robloxUsername} (ID: ${robloxData.robloxId})`);

  await discordLog('Profile Created', `**Roblox:** ${player.robloxUsername} (ID: ${player.robloxId})\n**Discord:** <@${player.discordId}>\n**Region:** ${player.region}`, 'success');

  refreshLeaderboard(interaction.guildId).catch((e) => logger.error('Background leaderboard refresh failed:', e));
}
